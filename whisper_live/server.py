...
from whisper_live.log import LOG_LEVEL
...
class ClientManager:
    ...
    def is_client_timeout(self, websocket):
        ...
        if elapsed_time >= self.max_connection_time:
            ...
            LOG_LEVEL.info(f"Client with uid '{self.clients[websocket].client_uid}' disconnected due to overtime.")
            return True
        return False
...
class TranscriptionServer:
    ...
    def __init__(self, max_client=4, max_connection_time=600):
        self.client_manager = ClientManager(max_clients=max_client, max_connection_time=max_connection_time)
    ...
    def initialize_client(
        ...
        if self.backend.is_tensorrt():
            try:
                ...
                LOG_LEVEL.info("Running TensorRT backend.")
            except Exception as e:
                LOG_LEVEL.error(f"TensorRT-LLM not supported: {e}")
        ...
        if self.backend.is_faster_whisper():
            if faster_whisper_custom_model_path is not None and os.path.exists(faster_whisper_custom_model_path):
                LOG_LEVEL.debug(f"Utilisation du model {faster_whisper_custom_model_path}")
            ...
            LOG_LEVEL.debug("Running faster_whisper backend.")
    ...
    def get_audio_from_websocket(self, websocket):
        ...
        if frame_data == b"END_OF_AUDIO":
            return False
        # LOG_LEVEL.info(f"Received audio data of length {len(frame_data)}")
        # Ajoute ce code pour sauvegarder les données audio reçues
        # with open("received_audio_data.raw", "ab") as f:
        #     f.write(frame_data)
    ...
    def handle_new_connection(self, websocket, faster_whisper_custom_model_path, whisper_tensorrt_path, trt_multilingual):
        """
        Initialise la connexion du client et vérifie si le serveur est prêt.
        Répond aux pings si nécessaire.

        Args:
            websocket: La connexion WebSocket du client.
            faster_whisper_custom_model_path: Chemin vers le modèle Faster-Whisper personnalisé.
            whisper_tensorrt_path: Chemin vers le modèle TensorRT.
            trt_multilingual: Si True, le modèle TensorRT est multilingue.
        ...
        try:
            ...
            LOG_LEVEL.error(f"Failed to retrieve client IP: {str(e)}")
        ...
        try:
            while True:
                ...
                # 1. Traiter les pings pour tester la connexion
                if self.handle_ping_message(websocket, options):
                    continue  # Passer au prochain message

                # 2. Vérifier si le message contient des options valides
                options = json.loads(options)
                self.use_vad = options.get('use_vad')
                ip_ch = options.get('ip_client', client_ip)
                LOG_LEVEL.info(f"New client connected :{ip_ch}")

                if self.client_manager.is_server_full(websocket, options):
                    websocket.close()
                    return False

                # Initialiser le client avec le modèle choisi
                if self.backend.is_tensorrt():
                ...
                return True  # Retourner True si l'initialisation réussit

        except json.JSONDecodeError:
            LOG_LEVEL.error(f"Failed to decode JSON from client with IP: {client_ip}->{ip_ch}")
            return False
        except ConnectionClosed as e:
            close_reason = e.reason
            LOG_LEVEL.debug(f"Connection closed by client with IP: {close_reason}")
            return False
        except Exception as e:
            LOG_LEVEL.error(f"Error during new connection initialization for client with IP: {client_ip}->{ip_ch}: {str(e)}")
            return False
    ...
    def process_audio_frames(self, websocket):
        ...
        if frame_np is False:
            if self.backend.is_tensorrt():
                client.set_eos(True)
            return False
        LOG_LEVEL.debug(f"[DEBUG]: Processing audio frame of size {frame_np.size}")
    ...
    def handle_ping_message(self, websocket, message):
        """
        Gère les messages de type ping et répond avec un pong si nécessaire.

        Args:
            websocket: La connexion WebSocket du client.
            message: Le message reçu du client (binaire ou texte).

        Returns:
            bool: True si le message était un ping et a été traité, sinon False.
        """
        # Vérifier si le message est textuel
        if isinstance(message, bytes):
            return False  # Ce n'est pas un message ping, continuer avec les données audio

        try:
            # Tenter de charger le message comme JSON
            message_data = json.loads(message)
            if "ping" in message_data and message_data["ping"] == "ping":
                ip_client = message_data.get("ip_client", "IP inconnue")
                LOG_LEVEL.debug(f"[PING]: Ping reçu depuis l'IP client : {ip_client}")
                websocket.send(json.dumps({"ping": "pong", "ip_client": ip_client}))
                return True  # Indique que le message a été traité comme un ping
        except (json.JSONDecodeError, TypeError):
            pass  # Ignorer si ce n'est pas un JSON valide
        return False
    ...
    def recv_audio(self,
        ...
        """
        Réception des trames audio après l'initialisation réussie.
        """
        self.backend = backend
        # Tester la connexion et initialiser le client
        ...
        try:
            while not self.client_manager.is_client_timeout(websocket):
                # Recevoir et traiter les trames audio
                if not self.process_audio_frames(websocket):
                    break
        except ConnectionClosed as e:
            close_raison = e.reason
            LOG_LEVEL.info(f"Connexion fermée par le client({close_raison}).")
        except Exception as e:
            LOG_LEVEL.error(f"Erreur inattendue : {str(e)}")
        finally:
    ...
    def run(self,
        ...
        if single_model:
            if faster_whisper_custom_model_path or whisper_tensorrt_path:
                LOG_LEVEL.debug("Custom model option was provided. Switching to single model mode.")
                self.single_model = True
                # TODO: load model initially
            else:
                LOG_LEVEL.debug("Single model mode currently only works with custom models.")
...
class ServeClientBase(object):
    ...
    def add_frames(self, frame_np):
        ...
        LOG_LEVEL.debug(f"[DEBUG]: Added frames to buffer, buffer size now {self.frames_np.size}")
    ...
    def send_transcription_to_client(self, segments):
        ...
        """
        try:
            message_info = segments[-1]
            if message_info.get('completed', False):
                LOG_LEVEL.info(message_info.get('text', 'VIDE'))
            self.websocket.send(
                json.dumps({
                    "uid": self.client_uid,
                    "segments": segments,
            ...
            if message_info.get('completed', False):
                LOG_LEVEL.error(f"[ERROR]: Sending data to client[{message_info.get('text', 'VIDE')}]: {e}")
            else:
                LOG_LEVEL.info(message_info.get('text', 'VIDE'))
...
class ServeClientTensorRT(ServeClientBase):
    ...
    def warmup(self, warmup_steps=10):
        ...
        """
        LOG_LEVEL.info("[INFO:] Warming up TensorRT engine..")
    ...
    def transcribe_audio(self, input_bytes):
        ...
        LOG_LEVEL.info(f"[WhisperTensorRT:] Processing audio with duration: {input_bytes.shape[0] / self.RATE}")
    ...
    def speech_to_text(self):
        ...
        while True:
            if self.exit:
                LOG_LEVEL.debug("Exiting speech to text thread")
                break
            ...
            try:
                ...
                LOG_LEVEL.info(f"[WhisperTensorRT:] Processing audio with duration: {duration}")
                self.transcribe_audio(input_sample)

            except Exception as e:
                LOG_LEVEL.error(f"[ERROR]: {e}")
...
class ServeClientFasterWhisper(ServeClientBase):
    ...
    def __init__(self, websocket, task="transcribe", device=None, language=None, client_uid=None, model="small.en",
        ...
        """
        super().__init__(client_uid, websocket)
        LOG_LEVEL.debug(f"Init de ServeClientFasterWhisper avec model={model}, language={language}")
        self.model_sizes = [
        ...
        if self.model_size_or_path is None:
            return
        LOG_LEVEL.debug(f"Using Device={device} with precision {self.compute_type}")
        if single_model:
        ...
        # threading
        self.trans_thread = threading.Thread(target=self.speech_to_text)
        LOG_LEVEL.debug("creat thread...")
        self.trans_thread.start()
        LOG_LEVEL.debug("starting thread...")
        LOG_LEVEL.debug(f"sending to {self.client_uid}...")
        self.websocket.send(
            json.dumps(
                {
                    "uid": self.client_uid,
        ...
        LOG_LEVEL.debug("sending uid ... Terminé")

    def create_model(self, device):
        """
        Instantiates a new model, sets it as the transcriber.
        """
        LOG_LEVEL.debug(f"creat_model() with model = {self.model_size_or_path}...")
        ...
        LOG_LEVEL.debug("creat_model()... Terminé")
    ...
    def set_language(self, info):
        ...
        if info.language_probability > 0.5:
            ...
            LOG_LEVEL.debug(f"Detected language {self.language} with probability {info.language_probability}")
            self.websocket.send(json.dumps(
        ...
        while True:
            ...
            try:
                ...
                if result is None or self.language is None:
                    self.timestamp_offset += duration
                    LOG_LEVEL.debug("no voice activity")
                    time.sleep(0.25)    # wait for voice activity, result is None when no voice activity
                    continue
                self.handle_transcription_output(result, duration)

            except Exception as e:
                # récuperer la trace complète de l'exception
                tb = traceback.format_exc()
                exc_type, exc_value, exc_traceback = traceback.extract_tb(e.__traceback__)[-1]
                LOG_LEVEL.error(f"Failed to transcribe audio chunk: {e} at {exc_traceback.lineno}\nTraceback:\n{tb}")
                time.sleep(0.01)
